package pl.lodz.uni.math.amazons.ai;
...
import pl.lodz.uni.math.amazons.config.Options;
import pl.lodz.uni.math.amazons.controller.MainWindowController;
import pl.lodz.uni.math.amazons.dto.TileDTO;
import pl.lodz.uni.math.amazons.dto.TreeNodeDTO;
import pl.lodz.uni.math.amazons.enums.AiGameResults;
...
public class MonteCarloAi {
    ...
    private TreeNodeDTO getMaxGameWins(List<TreeNodeDTO> nodes) {
        ...
        for (int i = 1; i < newNodes.size(); i++) {
            if (newNodes.get(max).getGameWins() * 1.0 / newNodes.get(max).getGamePlayed() < newNodes.get(i).getGameWins() * 1.0
