...
import pl.lodz.uni.math.amazons.ai.Ai;
...
import pl.lodz.uni.math.amazons.enums.PlayerType;
...
public class Game {
    ...
    private void move(TileDTO clicked) {
        if (select == null) {
            ...
            if (moves.contains(clicked)) {
                ...
                controller.addText("Move from " + select + " to " + clicked + ". ");
    ...
    private void shoot(TileDTO clickedTile) {
        if (moves.contains(clickedTile)) {
            controller.addText("Shoot to " + clickedTile + ".\n");
